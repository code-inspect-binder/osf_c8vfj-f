...
rm(list = setdiff(ls(), c("make_list", "SE_Within", "D1", "data.analysis.rt.exp2", "data.analysis.rt.exp3", "data.analysis.err.exp2", "data.analysis.err.exp3")))	
...
plot_rt <- barchart(RT ~ cond, groups = con, data = data.plot.rt, box.ratio = 3,
	ylim = c(527,593), scales = list(tck = c(1,0), x = list(labels = c("In-Group","Out-Group","In/Out-Group")), y = list(at = round(seq(530, 590, 15)))),
...
data.analysis.rt.exp2 <- data.analysis.rt
...
data.analysis.rt.plusiat <- data.frame(data.analysis.rt, D1 = D1$D1)
fit.rt <- lm(cbind(in_C,in_IC,out_C,out_IC,both_C,both_IC) ~ D1, data = data.analysis.rt.plusiat)
etasq(Anova(fit.rt, type = "III", test = "Wilks", idata = idata, idesign = ~cond*con), anova = T, partial = T)
etasq(Anova(fit.rt, type = "III", test = "Wilks", idata = idata, idesign = ~cond*con), anova = T, partial = T)$"approx F"

Lims <- conf.limits.ncf(F.value = 0.51, conf.level = 0.90, df.1 <- 2, df.2 <- 36)
...
Lims <- conf.limits.ncf(F.value = 3.54, conf.level = 0.90, df.1 <- 2, df.2 <- 37)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
...
my_d_inout <- print(ci.sm(sm = cohensD(data.analysis.rt$in_IC - data.analysis.rt$in_C, data.analysis.rt$out_IC - data.analysis.rt$out_C, method = "paired"), N = 39))

ttest.tstat(my_t_inout$stat, n1 = 39, simple = T)
TOSTone(m = my_d_inout$Standardized.Mean, mu = 0, sd = 1, n = 39, low_eqbound_d = -0.3, high_eqbound_d = 0.3)

###### IN vs. BOTH ######

my_t_inboth <- print(t.test(data.analysis.rt$both_IC - data.analysis.rt$both_C, data.analysis.rt$in_IC - data.analysis.rt$in_C, paired = T))
my_d_inboth <- print(ci.sm(sm = cohensD(data.analysis.rt$both_IC - data.analysis.rt$both_C, data.analysis.rt$in_IC - data.analysis.rt$in_C,, method = "paired"), N = 39))

ttest.tstat(my_t_inboth$stat, n1 = 39, simple = T)
TOSTone(m = my_d_inboth$Standardized.Mean, mu = 0, sd = 1, n = 39, low_eqbound_d = -0.3, high_eqbound_d = 0.3)

###### OUT vs. BOTH ######

my_t_outboth <- print(t.test(data.analysis.rt$both_IC - data.analysis.rt$both_C, data.analysis.rt$out_IC - data.analysis.rt$out_C, paired = T))
my_d_outboth <- print(ci.sm(sm = cohensD(data.analysis.rt$both_IC - data.analysis.rt$both_C, data.analysis.rt$out_IC - data.analysis.rt$out_C, method = "paired"), N = 39))

ttest.tstat(my_t_outboth$stat, n1 = 39, simple = T)
TOSTone(m = my_d_outboth$Standardized.Mean, mu = 0, sd = 1, n = 39, low_eqbound_d = -0.3, high_eqbound_d = 0.3)
...
ci.sm(sm = cohensD(data.analysis.rt.con$IC, data.analysis.rt.con$C, method = "paired"), N = 39)

Lims <- conf.limits.ncf(F.value = 71.19, conf.level = 0.90, df.1 <- 1, df.2 <- 38)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
...
Lims <- conf.limits.ncf(F.value = 1.46, conf.level = 0.90, df.1 <- 2, df.2 <- 37)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
...
# ppi <- 300
# tiff(filename = "MYPATH\\imi_exp2.tiff", compression = "zip", width = 7*2*ppi, height = 7*1*ppi, pointsize = 7*(9/5), res = ppi)
...
data.analysis.err.exp2 <- data.analysis.err
...
Lims <- conf.limits.ncf(F.value = 1.02, conf.level = 0.90, df.1 <- 2, df.2 <- 37)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
...
my_d_inout <- print(ci.sm(sm = cohensD(data.analysis.err$in_IC - data.analysis.err$in_C, data.analysis.err$out_IC - data.analysis.err$out_C, method = "paired"), N = 39))
...
my_d_inboth <- print(ci.sm(sm = cohensD(data.analysis.err$in_IC - data.analysis.err$in_C, data.analysis.err$both_IC - data.analysis.err$both_C, method = "paired"), N = 39))
...
my_d_outboth <- print(ci.sm(sm = cohensD(data.analysis.err$out_IC - data.analysis.err$out_C, data.analysis.err$both_IC - data.analysis.err$both_C, method = "paired"), N = 39))
...
ci.sm(sm = cohensD(data.analysis.err.con$IC, data.analysis.err.con$C, method = "paired"), N = 39)

Lims <- conf.limits.ncf(F.value = 12.81, conf.level = 0.90, df.1 <- 1, df.2 <- 38)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
...
Lims <- conf.limits.ncf(F.value = 3.21, conf.level = 0.90, df.1 <- 2, df.2 <- 37)
Lims$Lower.Limit/(Lims$Lower.Limit + df.1 + df.2 + 1)
Lims$Upper.Limit/(Lims$Upper.Limit + df.1 + df.2 + 1)
