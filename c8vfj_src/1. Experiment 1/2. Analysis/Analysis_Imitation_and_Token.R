...
make_list <- function(ppn){
	...
	names <- c("trialnr","ppn","age","gender","handpos","group","block","cue","finger","stim","color","con","xr","r","corr","rt","re")
	names(data) <- names
...
head(data.full)
data.full <- select(data.full, -one_of("trialnr","cue","finger","stim","color_left","xr","r","re"))
head(data.full)
tail(data.full)
...
data.full$group <- factor(data.full$group, levels = c(1,2), labels = c("in_blue","in_green"))
	#in-group
data.full$block <- factor(data.full$block + 1)
	#block number
data.full$con <- factor(data.full$con, levels = c(1,0), labels = c("C_IN", "IC_IN"))
...
data.token <- read.csv2("MYPATH\\Tokens.csv", header = T)

	###### PPN IN-GROUP	######

data.token$group <- rep(dcast(ppn + group ~ "n", data = data.full, value.var = "rt", length)$group, each = 2)

	###### HAND GROUP ######

data.token$hand <- ifelse(data.token$group == "in_blue", ifelse(data.token$color == "blue", "in", "out"), ifelse(data.token$color == "blue", "out", "in"))

	###### AVERAGE TOKENS ######

data.token$total <- apply(select(data.token, dist1:dist3), 1, function(x) mean(x, na.rm = T))

############################
######    ANALYSIS    ######
############################

data.analysis.token <- dcast(ppn ~ hand, data = data.token, value.var = "total", mean)
apply(data.analysis.token,2,mean)
apply(data.analysis.token,2,sd)

my_t <- print(t.test(data.analysis.token$"in", data.analysis.token$"out", paired = T))
ci.sm(sm = cohensD(data.analysis.token$"in", data.analysis.token$"out", method = "paired"), N = 39)
...
plot_rt <- barchart(RT ~ con, data = data.plot.rt, box.ratio = 3,
	ylim = c(549,581), scales = list(tck = c(1,0), x = list(labels = c("In-Group Congruent", "In-Group Incongruent")),y = list(at = round(seq(540, 580, 10)))),
...
rb_rt <- ggplot(data.plot.rt.ppn, aes(x = con, y = RT, fill = con)) +
  ...
  scale_x_discrete(name = NULL, labels = c("In-Group\nCongruent","In-Group\nIncongruent")) + 
...
# ggsave('rb_rt.png', rb_rt, width = 3, height = 3, scale = 1.25)
...
my_d <- print(ci.sm(sm = cohensD(data.analysis.rt$IC, data.analysis.rt$C, method = "paired"), N = 37))

	###### TEST NULL EFFECT ######

ttest.tstat(my_t$stat, n1 = 37, simple = T)
TOSTone(m = my_d$Standardized.Mean, mu = 0, sd = 1, n = 37, low_eqbound_d = -0.3, high_eqbound_d = 0.3)

	###### CORRELATION TOKEN TASK ######

data.analysis.rt.plustk <- join(data.analysis.rt, data.analysis.token, type = "inner", by = "ppn")
data.analysis.rt.plustk$imi_CE <- data.analysis.rt.plustk$IC_IN - data.analysis.rt.plustk$C_IN
data.analysis.rt.plustk$tk_diff <- data.analysis.rt.plustk$"in" - data.analysis.rt.plustk$out

cor.test(data.analysis.rt.plustk$tk_diff, data.analysis.rt.plustk$imi_CE )
	
	###### ADD DOT-PROBE ######

###### DATA ######	

data.analysis.rt.dotprobe.exp1$dp_CE <- data.analysis.rt.dotprobe.exp1$IC - data.analysis.rt.dotprobe.exp1$C
data.analysis.rt.plusdp <- join(data.analysis.rt, select(data.analysis.rt.dotprobe.exp1, ppn, dp_CE), type = "inner", by = "ppn")
data.analysis.rt.plusdp$imi_CE <- data.analysis.rt.plusdp$IC_IN - data.analysis.rt.plusdp$C_IN

###### CORRELATION ######

cor.test(data.analysis.rt.plusdp$dp_CE, data.analysis.rt.plusdp$imi_CE)
...
plot_err <- barchart(ErrorRate ~ con, data = data.plot.err, box.ratio = 3,
	ylim = c(1.9,5.1), scales = list(tck = c(1,0), x = list(labels = c("In-Group Congruent", "In-Group Incongruent")), y = list(at = seq(2, 5, 1))),
	ylab = "Error Rate (%)",
	auto.key = list(corner = c(0.975,.975), points = F, rectangles = T, height = 0.85, size = 3.5, padding.text = 2.5),
	par.settings = mytheme,
	panel=function(x, y, subscripts, stderr, box.ratio, ...){
...
# ppi <- 300
# tiff(filename = "MYPATH\\imi_exp1.tiff", compression = "zip", width = 7*2*ppi, height = 7*1*ppi, pointsize = 7*(9/5), res = ppi)
...
rb_err <- ggplot(data.plot.err.ppn, aes(x = con, y = ErrorRate, fill = con)) +
  geom_flat_violin(aes(fill = con),position = position_nudge(x = .1, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+
...
# ggsave('rb_err.png', rb_err, width = 3, height = 3, scale = 1.25)
	
	###### PRINT RT AND ER PLOTS ######
	
rb_both_plot <- plot_grid(rb_rt, rb_err, ncol = 2)
...
# ggsave('rb_rt+err.png', rb_both, width = 6, height = 3, scale = 1.25)	

	###### PRINT AI AND DP PLOTS ######
	
rb_all <- plot_grid(rb_both, rb_dp_both_exp1, ncol = 1, nrow = 2)
# ggsave('rb_ai+dp.png', rb_all, width = 6, height = 6)	
...
my_d <- print(ci.sm(sm = cohensD(data.analysis.err$IC, data.analysis.err$C, method = "paired"), N = 37))
